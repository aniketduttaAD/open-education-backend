...
export class VideoGenerationService {
  ...
  constructor(
    ...
    this.marpCliPath = 'marp'; // Hardcoded
    this.ffmpegPath = 'ffmpeg'; // Hardcoded
  ...
  async generateVideo(content: {
    ...
    try {
      ...
      const audioPath = await this.generateAudio(content.transcript, sessionDir);
      
      // Step 3: Compile video with FFmpeg
      const videoPath = await this.compileVideo(slidesPath, audioPath, sessionDir, content.title);
  ...
  private async checkBrowserAvailability(): Promise<boolean> {
    try {
      ...
      return new Promise((resolve) => {
        const chromeProcess = spawn('which', ['chromium-browser'], { stdio: 'ignore' });
   ...
   */
  private async generateSlides(slides: string[], sessionDir: string): Promise<string> {
    this.logger.log('Generating slides with Marp CLI');
    
    // Check browser availability
    const browserAvailable = await this.checkBrowserAvailability();
    if (!browserAvailable) {
      this.logger.warn('Browser not available, Marp CLI may fail. Ensure Chrome/Chromium is installed.');
    }
    ...
    return new Promise((resolve, reject) => {
      const marpProcess = spawn(this.marpCliPath, [
        markdownPath,
        '--images',
        'png',
        '--output-dir',
      ...
      marpProcess.on('close', (code) => {
        if (code === 0) {
          this.logger.log('Slides generated successfully');
          resolve(outputDir);
        } else {
          this.logger.error(`Marp CLI failed with code ${code}: ${errorOutput}`);
          
          // Provide helpful error message for browser-related issues
          if (errorOutput.includes('browser') || errorOutput.includes('chrome') || errorOutput.includes('chromium')) {
            reject(new Error(`Marp CLI failed: Browser not found or not accessible. Please ensure Chrome/Chromium is installed. Error: ${errorOutput}`));
          } else {
            reject(new Error(`Marp CLI failed: ${errorOutput}`));
          }
        }
      });
      
      marpProcess.on('error', (error) => {
        this.logger.error(`Marp CLI process error:`, error);
        reject(error);
      });
    });
  }
   ...
   */
  private async generateAudio(transcript: string, sessionDir: string): Promise<string> {
  ...
  private async compileVideo(
    ...
    const videoPath = path.join(sessionDir, `${title.replace(/[^a-zA-Z0-9]/g, '_')}.mp4`);
    
    return new Promise((resolve, reject) => {
      const command = ffmpeg()
        .setFfmpegPath(this.ffmpegPath)
        .input(slidesDir + '/slides-*.png')
...
`;
   ...
   */
  private async moveToOutput(videoPath: string, title: string): Promise<string> {
  ...
  private getVideoUrl(videoPath: string): string {
    // Return MinIO URL instead of local uploads path
    return videoPath;
  }
